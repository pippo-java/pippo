...
public class DefaultErrorHandler implements ErrorHandler {
    ...
    public void handle(int statusCode, Request request, Response response) {
        ...
        if (engine == null) {
            ...
            // default to an html template engine
            engine = application.getContentTypeEngine(HttpConstants.ContentType.TEXT_HTML);
        }

        if (engine == null) {
            renderDirectly(request, response);
        } else if (engine instanceof TemplateEngine) {
            String template = TemplateEngine.getTemplateForStatusCode(statusCode);
            try {
                Map<String, Object> locals = prepareLocals(statusCode, request, response);
                response.getLocals().putAll(locals);
                response.render(template);
            } catch (Exception e) {
                log.error(String.format("Unexpected error rendering your '%s' template!", template), e);
                renderDirectly(request, response);
            }
        } else if (engine instanceof RepresentationEngine) {
            try {
                Map<String, Object> locals = prepareLocals(statusCode, request, response);
                response.represent(locals, engine.getContentType());
            } catch (Exception e) {
                log.error(String.format("Unexpected error rendering generating '%s' representation!", acceptType), e);
                renderDirectly(request, response);
            }
        }
    }
